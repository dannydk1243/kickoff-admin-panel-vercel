...
import { getAllWithdrawals } from "@/components/dashboards/services/apiService"
import { getColumns } from "./columns"
import { WithdrawalTableToolbar } from "./data-table-toolbar"
import { WithdrawalStats } from "../../../../../pages/withdrawal/_components/WithdrawalStats"
...
export function WithdrawalDataTable() {
  ...
  const [walletData, setWalletData] = useState<any>(null) // ✅ NEW
  ...
  const updateWithdrawalList = async () => {
  ...
  return (
    <div className="space-y-8">
      <div className="max-w-5xl mx-auto w-full">
        <WithdrawalStats dictionary={dictionary} walletData={walletData} />
      </div>

      <Card>
        <CardHeader className="flex-row justify-between items-center gap-x-1.5 space-y-0">
          <CardTitle>{dictionary.tableLabels.withdrawalDataTable}</CardTitle>
          <WithdrawalTableToolbar
            withdrawableBalance={walletData?.withdrawableAmount}
            table={table}
            searchTerm={searchTerm}
            setSearchTerm={setSearchTerm}
            selectedStatus={selectedStatus}
            setSelectedStatus={setSelectedStatus}
            callback={updateWithdrawalList}
