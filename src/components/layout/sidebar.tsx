...
export function Sidebar({ dictionary, adminData }: SidebarProps) {
  ...
  function hasRoles(
    ...
    return Array.isArray((item as any).roles)
  }
  ...
  function canSeeMenuItem(
    ...
    // Items WITHOUT roles (like nested folders) are always visible
    if (!hasRoles(item)) return true

    // Items WITH roles but user has no role
              ...
              <SidebarGroup key={nav.title}>
                ...
                <SidebarGroupContent>
                  <SidebarMenu>
                    ...
                    {nav.items
                      .filter((item) => canSeeMenuItem(item, adminData?.role))
                      .map((item) => (
                        <SidebarMenuItem key={item.title}>
                          {renderMenuItem(item)}
                        </SidebarMenuItem>
