...
import { Album, Bell, Megaphone, MessageCircle, SquareMenu } from "lucide-react"
...
const getCategoryIcon = (category: string, target?: string) => {
  ...
  if (category === 'NOTIFICATION') {
    switch (target) {
      ...
      case 'BOOKING': return <Album className="h-4 w-4" />;
      default: return <Bell className="h-4 w-4 text-slate-500" />;
    }
  }

  return <Bell className="h-4 w-4" />; // Fallback
};
                  ...
                  >
                    <div className="flex items-start gap-3 py-4 px-4 hover:bg-accent transition-colors">
                      <Avatar className="h-9 w-9">
                        <AvatarFallback>
                          {getCategoryIcon(item.category, item.target)}
                        </AvatarFallback>
                      </Avatar>
